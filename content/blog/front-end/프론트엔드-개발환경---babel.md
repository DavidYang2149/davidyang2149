...
이렇게 한언어로 작성된 소스 코드를 비슷한 수준의 추상화를 가진 다른 언어로 변환하는 것을 **트랜스파일**이라고 표현한다.
...
바벨의 주된 사용용도는 _ES6(2015)_ 이상의 코드를 적당한 하위 버전으로 바꾸는 것이다.
...
target에 `ie: 11`을 설정하고 빌드한 것인데 *인터넷 익스플로러는 여전히 프라미스를 해석하지 못하고 🚨에러*를 던진다.
